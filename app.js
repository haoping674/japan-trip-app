...
const defaultExpensePayer = () => expenseMembers().find((member) => member.name === "鳳英")?.id || "";
const expensePayerOptions = (selectedPayer = "") => {
  const members = expenseMembers();
  const isKnownPayer = selectedPayer === "all" || selectedPayer === "全體" || members.some((member) => member.id === selectedPayer);
  const legacyPayerOption = selectedPayer && !isKnownPayer ? `<option value="${safe(selectedPayer)}" selected>${safe(expensePayerName(selectedPayer))}</option>` : "";
  return `${legacyPayerOption}${members.length ? members.map((member) => `<option value="${safe(member.id)}"${member.id === selectedPayer ? " selected" : ""}>${safe(member.name)}</option>`).join("") : `<option value="" disabled>成員載入中…</option>`}<option value="all"${selectedPayer === "all" || selectedPayer === "全體" ? " selected" : ""}>全體</option>`;
};
...
const openPhraseEditor = (phraseId = "") => {
  ...
  closePhraseEditor();
  app.insertAdjacentHTML("beforeend", `<div class="phrase-editor-modal"><div class="phrase-editor-modal__backdrop" data-phrase-editor-close></div><section class="phrase-editor-modal__sheet" role="dialog" aria-modal="true" aria-labelledby="phrase-editor-title"><div class="phrase-editor-modal__head"><div><small>SHARED PHRASEBOOK</small><h2 id="phrase-editor-title">${phraseId ? "編輯常用短語" : "新增常用短語"}</h2></div><button type="button" data-phrase-editor-close aria-label="關閉">×</button></div><p class="phrase-editor-modal__hint">只要輸入中文並選分類，日文與羅馬拼音會在儲存時自動產生；播放時會使用裝置的日語語音；羅馬拼音讀音來源：Yomitan／Romaji2Kana。儲存後同步給所有旅伴。</p><form class="phrase-editor-form" id="phrase-editor-form" data-phrase-id="${safe(phraseId)}"><label class="phrase-editor-field"><span>中文提示</span><input name="zh" required maxlength="80" value="${safe(phrase.zh)}" placeholder="例如：請問洗手間在哪裡？" /></label><label class="phrase-editor-field"><span>分類</span><select name="category">${categoryOptions}</select></label><div class="phrase-generated-preview"><small>自動生成內容</small><strong lang="ja">${safe(phrase.ja || "儲存後自動產生")}</strong><span>${safe(phrase.roma || "羅馬拼音會一起產生")}</span></div><p class="phrase-editor-error" aria-live="polite"></p><div class="phrase-editor-modal__actions"><button class="outline-action" type="button" data-phrase-editor-close>取消</button><button class="primary-button" type="submit">儲存並自動生成</button></div></form></section></div>`);
...
document.addEventListener("submit", (event) => {
  const expenseForm = event.target.closest("#expense-edit-form");
  if (expenseForm) {
    event.preventDefault();
    const expense = state.expenses.find((item) => item.id === expenseForm.dataset.expenseId);
    if (!expense) { closeExpenseEditor(); return; }
