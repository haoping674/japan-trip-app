...
let syncedMembers = Array.isArray(initial.members) ? initial.members : null;
...
const money = (value) => new Intl.NumberFormat("zh-TW", { style:"currency", currency:"JPY", maximumFractionDigits:0 }).format(value || 0);
...
function render() { const pages = { itinerary:itineraryPage, bookings:bookingPage, expenses:expensePage, planning:planningPage }; app.innerHTML = pages[state.section](); document.querySelectorAll(".bottom-nav__item").forEach((button) => button.classList.toggle("is-active", button.dataset.section === state.section)); }
